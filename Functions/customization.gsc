...
menuPosEditor()
{
    ...
    posEditor[6] = self createText("default", 0.7, "CENTER", "CENTER", self.presets["X"] + 130, self.presets["Y"] + 65, 3, 1, "*THIS MENU IS MULTI TABBED*", self.presets["TEXT"]);  
    posEditor[7] = self createText("default", 0.7, "CENTER", "CENTER", self.presets["X"] + 130, self.presets["Y"] + 75, 3, 1, "*BE CAUTIOUS WHEN CHOOSING POSITION*", self.presets["TEXT"]);  
    wait .2;
    ...
    while( !self MeleeButtonPressed() )
    {
        ...
        else if( self.actionSlotsPressed[ "dpad_left" ]  )
        {
            xPos -= 10;
            foreach( hud in posEditor )
                hud.x -= 10;
...
edit_position( posEditor, type, amount )
{
    foreach( hud in posEditor )
    {
        if( type == "x" )
